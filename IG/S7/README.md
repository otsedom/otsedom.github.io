...
Tras la sesión anterior que introducía aspectos básicos de three.js, esta sesión cubre distintas características que permiten aportar mayuor realismo a la escena. Como punto de partida, en 
*server.js* se invoca el archivo *index_S7.html*.
...
La razón es que no hay iluminación definida. Descomentando el bloque de código desde el comentario *//Luz ambiente* se activa una **luz ambiente** ([AmbientLight](https://threejs.org/docs/#api/en/lights/AmbientLight)) y su interfaz para modificar sus parámetros de color e intensidad. La iluminación de ambiente no tiene definida dirección, e intenta simular la luz que llega a la superficie de los objetos por reflexión difusa tras rebotar en todos los elementos de la escena, aspecto no contemplado en los modelos de iluminación/reflexión local. En la ejecución, observarán en la parte superior derecha, una pequeña GUI para modificar los parámetros de las luces, basada en [lul.gui](https://lil-gui.georgealways.com), que permite modificar parámetros de las distintas luces que se irán activando.
...
Por defecto, una luz direccional emite hacia el *(0, 0, 0)*, punto que puede modificarse con la propiedad *target*.
En el código del ejemplo, se crea un asistente *DirectionalLightHelper* que facilita interpretar su dirección de emisión. Como cualquier luz, que no sea de ambiente, afectará a la superficie dependiendo del ángulo entre la normal a la superficie del objeto y la dirección de la fuente de luz.
...
por medio de la propiedad [*.shadowMap*](https://threejs.org/docs/#api/en/renderers/WebGLRenderer.shadowMap). En la documentación y ortras fuentes como los tutoriales de [SBCODE](https://sbcode.net/threejs/shadows/) puedes comparar entre los distintos tipos existentes. Para activar las sombras en el *renderer*:
...
Para la escena del ejemplo con tres esferas, interesa definir al menos un objeto que reciba las sombras, ya que de momento, se ha establecido que las esferas producen sombras, pero ningún objeto ha aactivado que las reciba. En el ejemplo, se crea una
...
Recordar que con anterioridad deben subirse las imágenes de los mapas de texturas al proyecto Glitch a través de la carpeta de *Assets*.
...
La tarea propuesta pide crear un sistema planetario con al menos cinco planetas y alguna luna, pudiendo alternar entre al menos dos vistas: vista general y vista subjetiva desde una *nave*. 
...
- Proporcionar pequeños vídeos que ilustren el funcionamiento
- Interfaz autocompleta, no es necesario acceder a documentación para manejarla
