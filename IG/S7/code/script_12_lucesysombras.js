...
function init() {
  ...
  const SueloMaterial = new THREE.MeshPhongMaterial({ color: 0xfafafa });
  const Suelo = new THREE.Mesh(SueloGeometry, SueloMaterial);
  Suelo.position.y = -5;
  if (sombra) Suelo.receiveShadow = true;
  scene.add(Suelo);
  ...
  Ldir_Info.addColor(Ldir_Params, "color")
    .name("Color")
  ...
  Lpunt_Info.add(Lpunt, "intensity", 0, 1, 0.1).name("Intensidad");
