...
Este guion centra la atención en el *shader* de fragmentos, siguiendo el esquema propuesto en [The Book of Shaders](https://thebookofshaders.com). Adopto un shader de vértices mínimo. <!--, con excepción de una leve mención en la sección dedicada a imágenes.-->
...
let uniforms = {}
  ...
  function init() {
    ...
    EsferaShader(1.5,0,0,0.8,10,10, 0xffff00)
...
Con un esqueleto mínimo para establecer el *shader*, en un nuevo ejemplo, se envía desde el código JavaScript las dimensiones de la ventana, las coordenadas del puntero, y el tiempo transcurrido desde el inicio de la ejecución. En este caso además he añadido como parámetro de entrada en la función *EsferaShader* la función que se asocia al *shader* de fragmentos.
...
function init() {
  ...
  EsferaShader(0, 0, 0, 1, 10, 10, fragmentShader_02());
  //Color dependiente de posición del fragmento
  EsferaShader(3.0, 0, 0, 1, 10, 10, fragmentShader_03());
...
El código crea tres esferas con distinto *shader* de fragmentos. También define la estructura *uniforms* que se utiliza para proporcionar datos desde programa al *shader*. La primera versión es como en ejemplo anterior, asigna un color a todo el objeto.
...
El segundo *shader* modifica el color en función del tiempo, una sinusoidal controla la componente azul, las componentes rojas y verde vienen afectadas por la posición del puntero.
...
En el tercero el color de la esfera viene dado por la posición en la ventana del píxel, normalizando por las dimensiones de la ventana. La esfera se verá con distintos tonos según donde esté en la ventana. Ten presenta que no hay luces definidas.
...
Si las variables globales a todos los píxeles son  *uniform*, variables como *gl_FragCoord*, aquellas que para cada hilo contienen el valor particular del píxel, son  *varying*.
...
Como base sobre la que trabajar, hago uso de una geometría *PlaneBufferGeometry*, aplico dimensiones para que ocupe toda la ventana. Las dimensiones pueden variar según la configuración de cámara y resolución de pantalla.
...
El *shader* del listado a continuación, modifica el anterior, pintando en verde cuando *st.x* y *st.y* tienen valores próximos, es decir en la diagonal. La condición de proximidad viene definida por la variable *grosor*.
...
El repertorio de funciones con las que jugar es más amplio. En el código GLSL, se muestra un abanico de ellas, basta comentar y descomentar la línea de código correspondiente para obtener distintos resultados en la forma dibujada durante la ejecución.
