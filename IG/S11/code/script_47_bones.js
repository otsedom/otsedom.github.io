...
function creaObjeto() {
  ...
  const nsegs = 8; //número de elementos
...
function animationLoop() {
  ...
  for (var i = 0; i < worm.skeleton.bones.length; i++) {
    ...
    worm.skeleton.bones[i].rotation.z = degreesToRadians(
      5 * i * Math.sin(1.9 * time - i)
    ); //prueba sin -i
    //Leve torsión sobre x más lenta y suave
    worm.skeleton.bones[i].rotation.x = degreesToRadians(
      i * Math.cos(0.2 * time + i * i)
    );
  }

  //Efectos extras sobre la parte inicial
